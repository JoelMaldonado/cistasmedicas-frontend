  ...
  <div class="page">
    ...
    <div class="calendar-scroll">
      <div class="calendar" :style="{ '--day-count': weekDays.length }">
        ...
        <div
          ...
          :class="{ 'calendar__day-header--today': day.isToday }"
        >
        ...
        <template v-for="hour in hours" :key="hour">
          ...
          <template v-for="day in weekDays" :key="`${day.date}-${hour}`">
            ...
            <div
              ...
              :class="{ 'cell--past': isPastCell(day.date, hour) }"
            />
          </template>
        </template>
      </div>
    ...
    <Popover ref="popover">
      <div v-if="selectedAppointment" class="popover-content">
        <p class="popover-content__patient">
        ...
        <div class="popover-content__actions">
          <template v-if="getEffectiveStatus(selectedAppointment) === 'pending'">
            <Button
              ...
              size="small"
              severity="success"
              @click="openAction('confirm', selectedAppointment)"
            />
            <Button
              label="Rechazar"
              size="small"
          ...
          <template v-else-if="getEffectiveStatus(selectedAppointment) === 'confirmed'">
            <Button
              ...
              severity="info"
              text
              @click="openReschedule(selectedAppointment)"
            />
            <Button
              label="Cancelar"
              size="small"
              severity="danger"
              text
              @click="openAction('cancel', selectedAppointment)"
            />
          </template>
        </div>
      </div>
    ...
    >
      <p class="reschedule-info">
        Selecciona un nuevo horario disponible para
